...
The first template `SriovOperatorConfig.yaml` is not overriding any settings, we can tell this because we don't have a `spec` or `metadata` section below the `policyName`. That means that the final policy will be configured as the https://github.com/openshift-kni/cnf-features-deploy/blob/release-4.12/ztp/source-crs/SriovOperatorConfig.yaml[source template file].

The second template `PerformanceProfile.yaml` is overriding some parameters in the `spec`. That means that the final policy will be configured as a merge of the https://github.com/openshift-kni/cnf-features-deploy/blob/release-4.12/ztp/source-crs/PerformanceProfile.yaml[original template] plus the custom overrides we provided in the PolicyGenTemplate.
