...
As we saw in an earlier section of the lab, we can leverage RHACM policies to get our clusters configured the way we want. We also introduced the PolicyGen component which helps us to create a set of policies based on pre-backed templates crafted by the Red Hat CNF team.
...
Explain how it works (templates), put examples
