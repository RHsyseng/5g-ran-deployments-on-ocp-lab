...
Actually, everything can be done from the Red Hat ACM multicloud console. Therefore, browse to https://multicloud-console.apps.hub.5g-deployment.lab, click on Infrastructure -> Clusters and then click on the three dots of our SNO1 cluster. Notice that the status of the cluster is **detached**. Now, click on Import cluster:
