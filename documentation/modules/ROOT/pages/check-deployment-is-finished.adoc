...
Now that SNO deployment is finished, automation should run as soon as the cluster is marked with the `ztp-done` label. We can easily check the labels on our `sno2` cluster via the command line or OpenShift web console.
...
Again, we can monitor policy compliance from either the command line or web console.
