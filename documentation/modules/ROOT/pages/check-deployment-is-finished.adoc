...
NOTE: Deployment of the hub cluster was not part of this lab, but if `local-cluster` was deployed via ZTP workflow as well, our automation policy will bind to it and run automation on `local-cluster` as well. Because of this behavior, utilizng ZTP and AAP automation together in a 5G RAN environment maintains ease of configuration at scale.
