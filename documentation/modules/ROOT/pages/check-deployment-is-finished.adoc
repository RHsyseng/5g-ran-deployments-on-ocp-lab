...
2. Once you're in, click on `Infrastructure` -> `Clusters`. You will see a screen like the one below, notice how there is a label saying `ztp-done` (that means ztp pipeline has finished):
+
...
[source,bash,subs="attributes+,+macros"]
-----
oc get managedcluster sno2 --show-labels
