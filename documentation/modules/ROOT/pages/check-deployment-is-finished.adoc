...
2. Once you're in, click on `Infrastructure` -> `Clusters`. You will see a screen like the one below, check at the SNO2 cluster if there is a label saying `ztp-done` (that will indicate that ztp pipeline has finished):
+
image::acm-sno2-ztp-done.png[RHACM SNO2 ztp-done]
+
3. Additionally, we can verify that all policies are compliant for our SNO cluster. On the RHACM WebUI go to `Governance` -> `Policies` and you will see the following screen where we can see that all the policies are green:
+
