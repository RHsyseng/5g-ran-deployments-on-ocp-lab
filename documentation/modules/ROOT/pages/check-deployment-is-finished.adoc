...
NOTE: Deployment of the hub cluster was not part of this lab, but if `local-cluster` was deployed via ZTP workflow, it will have a `ztp-done` label and our policy will run automation on `local-cluster` as well. This model of maintaing one automation policy for day 2 operations for N clusters makes utilizng ZTP and AAP automation together in a 5G RAN environment great at scale.
