...
-----
+
3. Include `openshift-adp` namespace in the list of namespaces managed by the GitOps operator.
+
[.console-input]
[source,bash,subs="attributes+,+macros"]
-----
...
Above command creates the required Argo CD Applications in the Hub cluster that handles the ZTP Pipeline.
...
You can access the Argo CD instance on the lab at {argocd-hub-link} and login in with the OpenShift admin credentials. You can also access through the pre-configured Firefox image:bookmark-argocd.png[ArgoCD bookmark icon] bookmark.

You can see what the ArgoCD Apps are running in the following images:
