...
[#upload-collections-to-aap-hub]
=== Upload Necessary Ansible Collections to AAP Hub

An AutomationHub, a private Ansible Galaxy server for storing Ansible collections, is deployed on our OpenShift cluster. AutomationHub is a downstream version of link:https://github.com/ansible/galaxy_ng[Galaxy NG]. We need to upload the collections we will use in this lab.

First, retrieve Automation Hub URL and credentials, and generate an authentication token via the Automation Hub's API.
...
We will also need to add the following entry to /etc/hosts:
...
[source,bash]
-----
curl https://raw.githubusercontent.com/ewsiegel/5g-ran-deployments-on-ocp-lab/aap-integration-updates/aap-integration-tools/collections/collections_to_hub.bash -o ~/collections_to_hub.bash
-----
...
After running this script, if we head to automation-hub-aap.apps.hub.5g-deployment.lab and click on the "Collections" tab, we will see the collections we have just uploaded.
