...
First, set our target kubeconfig and wait for AutomationHub and AutomationController to finish deployment.

[.console-input]
[source,bash]
-----
export KUBECONFIG=~/.kcli/clusters/hub/auth/kubeconfig
oc -n aap wait deployment.apps/automation-controller-operator-controller-manager --for=jsonpath='{.status.readyReplicas}'=1 --timeout=900s
oc -n aap wait deployment.apps/automation-hub-api --for=jsonpath='{.status.readyReplicas}'=1 --timeout=900s
oc -n aap wait deployment.apps/automation-hub-content --for=jsonpath='{.status.readyReplicas}'=2 --timeout=900s
oc -n aap wait deployment.apps/automation-hub-operator-controller-manager --for=jsonpath='{.status.readyReplicas}'=1 --timeout=900s
oc -n aap wait deployment.apps/automation-hub-redis --for=jsonpath='{.status.readyReplicas}'=1 --timeout=900s
oc -n aap wait deployment.apps/automation-hub-worker --for=jsonpath='{.status.readyReplicas}'=2 --timeout=900s
oc -n aap wait deployment.apps/automation-task --for=jsonpath='{.status.readyReplicas}'=1 --timeout=900s
oc -n aap wait deployment.apps/automation-web --for=jsonpath='{.status.readyReplicas}'=1 --timeout=900s
oc -n aap wait deployment.apps/eda-server-operator-controller-manager --for=jsonpath='{.status.readyReplicas}'=1 --timeout=900s
oc -n aap wait deployment.apps/resource-operator-controller-manager --for=jsonpath='{.status.readyReplicas}'=1 --timeout=900s
-----
