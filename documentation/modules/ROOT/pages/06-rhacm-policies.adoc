...
As we mentioned in the https://github.com/RHsyseng/5g-ran-deployments-on-ocp-lab/blob/main/content/4.md#ztp-workflow[ZTP Workflow] section, the Red Hat Advanced Cluster Management (RHACM) for Kubernetes allows us to _provision_ and _configure_ multiple clusters in a declarative way and can be easily coupled with GitOps. ZTP makes intensive use of RHACM Policies for managing the configuration of deployed clusters. In our telco use case, creation of these `Policies` is simplified with the ZTP policyGen kustomize plugin. This plugin translates the vRAN DU profile based on `PolicyGenTemplates` into multiple `Policies` that are understood by the Hub cluster and applied to the managed clusters.

In this section we provide an introduction to the concepts behind the RHACM “Governance" pillar (section). Furthermore, we are going to introduce the concept of a Policy, which is key for managing the configuration lifecycle of our managed clusters.
...
At a glance, once we create a Policy in the hub cluster, the Governance Policy framework is responsible for propagating it to set of target managed clusters based on a set of binding rules for the Policy. Then, the policy rules are evaluated against the current state of the managed cluster. The results of this assessment are sent back to the hub cluster and depending on the needs, we can remediate the drift or just be informed.

[#policies]
== Policies

Policy CRs, as defined by the Policy Custom Resource Definition (CRD), are created on the hub cluster and act as a vehicle to deliver policies to managed cluster and collect results to send to the hub cluster.
...
As we will detail in the following section [..] `PolicyGenTemplates` are by default set to generate Policies set to inform mode. 
