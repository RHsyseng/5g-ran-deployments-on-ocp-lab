...
In this https://github.com/openshift-kni/cnf-features-deploy/blob/release-4.13/ztp/gitops-subscriptions/argocd/example/siteconfig/example-sno.yaml[link] you can find an example of a SiteConfig for a SNO deployment. In the next section, we will create our own SiteConfig to deploy a SNO in our lab environment.
...
IMPORTANT: In 4.13 release a couple of noticeable changes in provisioning are included. Improvements such as the chance to use crun as the container runtime or leverage Composable OpenShift feature to reduce the platform footprint.
...
spec:
  releaseImage: infra.5g-deployment.lab:8443/openshift/release-images:4.12.3-x86_64 #TODO
EOF
-----
