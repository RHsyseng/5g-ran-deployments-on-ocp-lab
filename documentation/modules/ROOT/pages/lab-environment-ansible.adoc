...
IMPORTANT: There are variables that might be adjusted for your environment. They can be included as variables in the ansible-playbook command-line or a suggested included in the `run-role.yaml` ansible playbook.
...
The `run-role.yaml` playbook basically includes the `ocp4_workload_5gran_deployments_lab` Ansible role and adjustes the variables to the proper environment where the lab is about to be deployed. The following playbook probably will just work for you except for the `ocp4_pull_secret` which has to be replaced by your personal OpenShift Pull Secret.

The default upstream DNS is set to 1.1.1.1. There might be cases in your local environment where the hypervisor may not reach it. So, notice that you must change it to a different DNS that allows you to resolve public hostnames. 
...
- name: telco 5G lab
  ...
  vars:
    ...
    lab_sno_vm_disk: 120
    #upstream_dns: "10.6.73.2" #internal corporate DNS
    upstream_dns: "1.1.1.1" #internet DNS
    ACTION: provision
...
Next, create the inventory file in that path and copy and paste the `run-role.yaml` playbook shown previously. Finally run the playbook:
...
The Ansible playbook uses the mentioned role to deploy the lab. The role is clearly divided into 2 stages: `pre-workload` tasks and `workload` tasks:

[#preworkload-tasks-overview]
=== Pre-Workload Tasks Overview

The pre_workload.yml[https://github.com/RHsyseng/agnosticd/blob/development/ansible/roles_ocp_workloads/ocp4_workload_5gran_deployments_lab/tasks/pre_workload.yml] file contains the initial setup and preparation tasks that must be completed before deploying the 5G RAN lab environment. This file ensures your hypervisor is properly configured with all necessary dependencies, services, and infrastructure components. 
...
|---------|------|---------|-------------|
| **Container Registry** | 8443 | Stores OpenShift images | admin / r3dh4t1! |
| **Git Server (Gitea)** | 3000 | Lab repository access | student / student |
| **S3 Storage (MinIO)** | 9002 | Object storage | admin / admin1234 |
| **Web Cache** | 8080 | Image caching | - |
| **Redfish API** | 9000 | BMC simulation | - |
| **Lab Showroom** | 80 | Lab documentation | lab-user / redhat123 |
...
Once workload completes successfully, the system provides:
...
The workload phase creates a complete 5G RAN RDS lab environment ready for advanced telco workload deployment and testing.
