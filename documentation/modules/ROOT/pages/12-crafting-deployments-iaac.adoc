...
1. Login into the http://infra.5g-deployment.lab:3000/[Git server]
...
spec:
  ...
  clusterImageSetNameRef: "openshift-v4.11-disconnected"
...
-----
[user@workstation ~]$ git add --all
[user@workstation ~]$ git commit -m 'Added SNO2 and 5GLab Site information'
[user@workstation ~]$ git push origin main
-----