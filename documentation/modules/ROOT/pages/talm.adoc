...
The answer is that TALM interacts with ZTP for newly created clusters. When we define one or multiple spoke clusters in telco, we do not only want to provision them, we also want to apply a specific configuration such as the validated link:policygen-deepdive#5g-ran-profile[5G RAN DU profile]. Often we also want our workloads to be running on top of them. ZTP is envisioned as a streamlined process, whereas a developer we only push manifest definitions to the proper Git repository.
...
A ZTP wave is basically an annotation (ran.openshift.io/ztp-deploy-wave) included on each policy template that will permit TALM to apply policies in an ordered manner. As an example, every link:policygen-deepdive#policies-templating[policy] generated from a PolicyGenTemplate by ZTP incorporates a wave number such as the link:https://github.com/openshift-kni/cnf-features-deploy/blob/master/ztp/source-crs/SriovSubscriptionNS.yaml[SriovSubscriptionNS.yaml]:
...
IMPORTANT: Detailed information on policy templates can be found in the following link:policygen-deepdive#policies-templating[PolicyGen deepdive] section.
...
In case, the upgrade failed in a spoke cluster, the TALM CGU needs to be deleted in the hub cluster and an admin needs to log in to the spoke cluster to start the recovery process. The process is detailed in the https://github.com/openshift-kni/cluster-group-upgrades-operator/tree/main/docs/backup-recovery#recovery-from-upgrade-failure[Recovery from Upgrade failure] documentation.
