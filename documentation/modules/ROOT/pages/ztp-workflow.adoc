...
* *Seed cluster*. Dedicated single-node OpenShift cluster that you use to create a seed image and is deployed with the target OpenShift Container Platform version.
* *Lifecycle Agent Operator*. Generates the seed image.
* *SiteConfig Operator*. Creates the installation ISO, and embeds the seed image URL in the live installation ISO. It also attaches the Installation ISO into the host about to provision.
...
* Use the SiteConfig operator (ClusterInstance) or openshift-install program to embed the seed image URL, and other installation artifacts, in a live installation ISO.
...
* When the preinstallation completes, the host is ready to for rapid reconfiguration and deployment.
...
* Attach the Configuration ISO to the host and let the reconfiguration process to finish.
