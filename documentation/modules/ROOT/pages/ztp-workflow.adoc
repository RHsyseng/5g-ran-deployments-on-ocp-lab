...
As shown in the previous image, the Preinstall host is installed in a central location, and then we get the host configured using the SiteConfig operator. In this lab we cover how to create the preinstall host via the `openshift-install` binary as well as how to configure it using the SiteConfig operator. Similar to the link:ztp-workflow.html#abi-deployment[Agent Based Installation], the SiteConfig Operator transforms the ClusterInstance CR into the following Installation Manifests:
...
* Install a single-node OpenShift cluster with the target OpenShift version you want to install in the rest of the future clusters. #TODO: Explain how to install this seed cluster and the requirements
