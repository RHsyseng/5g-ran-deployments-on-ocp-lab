...
The following picture describe the process we are going to follow to deploy and configure the *sno-ibi* OpenShift Clusters with the RAN DU profile configured:
...
* The installation ISO is mounted remotely to the sno-ibi and the preinstallation process starts.

Once the preinstallation is finished, the sno-ibi will be shutdown. At this point, in a telco RAN scenario, the thousands of SNO clusters that were preinstalled in the customer premises will be transported to the remote locations. In this lab, once the sno-ibi arrives to the remote location, the SNO is ready to be reconfigured and complete the installation.
...
* The Lifecycle-Agent Operator that was included in the seed image and so it is available in the sno-ibi, detects the configuration ISO and starts the reconfiguration process until the OpenShift Cluster is completely deployed.

[#baremetal-node-details]
== Bare Metal Node Details

The details for our baremetal node that we want to provision as sno-ibi are the ones below:
...
At this stage we are ready to manually boot the sno-ibi with the Installation ISO we created. But, first we have to move the ISO to a place where the host about to install can grab it. Let's copy it to the link:lab-environment-introduction.html#webserver[HTTP server] we have available in our infrastructure:
...
Then, the sno-ibi will boot and the preinstallation process will start. We can take a look at the process by connecting to the sno-ibi via ssh. Something similar to the following output will be shown in the terminal. 

WARNING: The pre-installation process is finished once the sno-ibi is automatically powered off.
...
When we applied the ZTP GitOps Pipeline configuration in the last section, that made the `clusters` ArgoCD application to automatically synchronize the commited changes. Basically, this means, ArgoCD starting doing its magic so the cluster deployment should already be running, let's see what happened in Argo CD.
...
+
5. You can see how the pipeline created all the required objects to get `sno-ibi` deployed. Notice as well that the `sno-abi` objects are there from link:installing-agentbasedinstall-cluster.html#deploying-sno-using-gitops-pipeline[previous configuration].
...
Let's see how the OpenShift installation is progressing in the target cluster. First, we need the sno-ibi credentials. From the hub cluster we can obtain them using this command:
[.console-input]
[source,bash,subs="attributes+,+macros"]
-----
...
Now, that we have the sno-ibi admin credentials, let's see the current status of the cluster and how the different cluster operators are moving forward:
...
Notice that the Red Hat ACM console will show the sno-ibi OpenShift cluster as Ready:
...
At this stage the sno-ibi is deployed and configured with the RAN DU profile, ready to run telco CNF applications.