...
WARNING: The pre-installation process is complete once the sno-ibi is automatically powered off. Do **not** create or push the ClusterInstance / site-config manifests until that power-off happens; doing so attaches the configuration ISO too early and leaves the disk unbootable.
...
You can also confirm the host is powered off from the infrastructure node before continuing:

[.console-input]
[source,bash,subs="attributes+,+macros"]
-----
kcli list vm | grep sno-ibi
-----
...
spec:
  ...
  extraAnnotations:
    ImageClusterInstall:
      imageclusterinstall.extensions.hive.openshift.io/install-timeout: "3h"
  extraLabels:
    ManagedCluster:
...
configMapGenerator:
  - files:
    ...
    - reference-manifest/{lab-major-version}/10-rename-gnrd-interfaces-master.yaml
    - reference-manifest/{lab-major-version}/10-rename-gnrd-interfaces-worker.yaml
...
In a real baremetal environment, the installation process typically takes less than *10 minutes*. In this lab, it can take up to 1h (due to disk I/O limitations). You can confirm that the reconfiguration was successful by running the following command from the hub cluster.
