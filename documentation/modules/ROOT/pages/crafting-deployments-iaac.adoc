...
Now that we have a repository ready to be used we will clone it in our workstation:
...
mkdir -p site-configs/pre-reqs/sno2
mkdir -p site-policies/{common-policies,group-policies,site-policies,zone-policies}
touch site-configs/pre-reqs/sno2/.gitkeep
touch site-policies/{common-policies,group-policies,site-policies,zone-policies}/.gitkeep
git add --all
...
[source,bash,subs="attributes+,+macros"]
-----
cat <<EOF > ~/5g-deployment-lab/ztp-repository/site-configs/5glab.yaml
---
