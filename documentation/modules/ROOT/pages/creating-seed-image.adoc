...
[source,bash,subs="attributes+,+macros"]
-----
oc --kubeconfig {seed-cluster-kubeconfig} get seedgenerator seedimage -o json | jq '.status'
-----
...
[source,bash,subs="attributes+,+macros"]
-----
oc --kubeconfig {seed-cluster-kubeconfig} get seedgenerator
-----
...
At this point, the seed image is created and pushed to our container registry under `infra.5g-deployment.lab:8443/ibi/lab5gran:{sno-cluster-version1}`. The seed image is ready to be used to provision new SNO clusters using the image-based installation method.