...
* Create a seed image using the Lifecycle Agent. As mentioned, the seed image `infra.5g-deployment.lab:8443/ibi/lab5gran:{sno-cluster-version2-cvo}` is already available in the link:lab-environment-introduction.html#container-registry[container registry].
...
If we check the operators installed in `sno-abi`, we'll notice that neither LCA nor OADP are installed. This is expected because, as we saw in the link:crafting-cluster-telco-related-infra-operators-configs.html#crafting-common-policies[Crafting Common Policies] section, only SR-IOV and LVM operators were installed as Day 2 Operators.
...
IMPORTANT: An S3-compatible storage server is required for backup and restore operations. See the link:s3-storage[S3 Storage Server] section.
