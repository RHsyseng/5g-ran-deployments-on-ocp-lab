...
metadata:
  ...
  namespace: openshift-adp
  annotations:
    # Below annotation makes LCA label the configmap and adds a label selector to the backup (since we have secrets with dynamic names we need to backup everything for now)
