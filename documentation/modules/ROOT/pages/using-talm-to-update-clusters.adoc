...
In the previous sections we have deployed the `sno2` cluster and attached the `sno1` cluster. Before we continue with TALM, let's grab the kubeconfigs for both cluster since we will need them for the next sections. The following commands need to be only executed in the hypervisor as the root user.

WARNING: Be sure you are executing the following commands after becoming root in the hypervisor. Notice that when becoming root you can connect to the hub cluster using the `oc` binary.

[.console-input]
[source,bash,subs="attributes+,+macros"]
-----
oc --kubeconfig ~/hub-kubeconfig -n sno1 extract secret/sno1-admin-kubeconfig --to=- > ~/sno1-kubeconfig
...
WARNING: Be sure you are executing the following commands in the hypervisor. Notice that pre-cache job can take up to 5m to be created.
...
[source,bash,subs="attributes+,+macros"]
-----
oc --kubeconfig ~/sno2-kubeconfig logs job/pre-cache -n openshift-talo-pre-cache -f
-----
...
WARNING: The following commands are executed from the workstation. 
...
WARNING: Be sure you are executing the following commands in the hypervisor. Notice that backup job can take up to 5m to be created.

[.console-input]
[source,bash,subs="attributes+,+macros"]
-----
oc --kubeconfig ~/sno1-kubeconfig get jobs -A
-----
...
[source,bash,subs="attributes+,+macros"]
-----
oc --kubeconfig ~/sno2-kubeconfig logs job/backup-agent -n openshift-talo-backup -f
-----
...
WARNING: The following command is executed from your laptop or workstation. 
...
WARNING: Be sure you are executing the following commands in the hypervisor.

[.console-input]
[source,bash,subs="attributes+,+macros"]
-----
oc --kubeconfig ~/sno2-kubeconfig get clusterversion,nodes
-----
...
Notice that now the upgrade policy (europe-snos-upgrade-version-412-5) is now compliant on both clusters. See that, in order to save resources, the enforce policy is removed once the CGU is successfully applied. And finally, the CGU will be `Completed`:
