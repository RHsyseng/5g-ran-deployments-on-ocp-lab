...
WARNING: It's possible that you get `UpgradeNotCompleted`, if that's the case you need to wait for the remaining policies to be applied.
...
IMPORTANT: All the steps in the below section must be run as `root` user on the hypervisor host.

[.console-input]
[source,bash,subs="attributes+,+macros"]
-----
oc --kubeconfig ~/sno1kubeconfig get jobs -A
