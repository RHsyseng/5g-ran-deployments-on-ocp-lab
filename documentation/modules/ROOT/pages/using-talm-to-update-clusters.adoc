...
openshift-logging                      cluster-logging.v6.5.0                        Red Hat OpenShift Logging   6.5.0                            Succeeded
openshift-operator-lifecycle-manager   packageserver                                 Package Server              0.0.1-snapshot                   Succeeded
openshift-ptp                          ptp-operator.v4.21.0-202604200440             PTP Operator                4.21.0-202604200440              Succeeded
openshift-sriov-network-operator       sriov-network-operator.v4.21.0-202604200818   SR-IOV Network Operator     4.21.0-202604200818              Succeeded
openshift-storage                      lvms-operator.v4.21.0                         LVM Storage                 4.21.0                           Succeeded
-----
...
-----
NAMESPACE                              NAME                                          DISPLAY                     VERSION               REPLACES                  PHASE
openshift-adp                          oadp-operator.v1.5.5                          OADP Operator               1.5.5                 oadp-operator.v1.5.4      Succeeded
openshift-lifecycle-agent              lifecycle-agent.v4.21.2                       Lifecycle Agent             4.21.2                lifecycle-agent.v4.21.1   Succeeded
openshift-logging                      cluster-logging.v6.5.0                        Red Hat OpenShift Logging   6.5.0                                           Succeeded
openshift-operator-lifecycle-manager   packageserver                                 Package Server              0.0.1-snapshot                                  Succeeded
openshift-ptp                          ptp-operator.v4.21.0-202604200440             PTP Operator                4.21.0-202604200440                             Succeeded
openshift-sriov-network-operator       sriov-network-operator.v4.21.0-202604200818   SR-IOV Network Operator     4.21.0-202604200818                             Succeeded
...
IMPORTANT: ACM Observability creates the Alertmanager secrets in `openshift-monitoring` with a suffix derived from the hub cluster ID. The Lifecycle Agent `lca.openshift.io/apply-label` annotation expects the unsuffixed names, so create aliases from the suffixed secrets:

[.console-input]
[source,bash,subs="attributes+,+macros"]
-----
oc --kubeconfig {abi-cluster-kubeconfig} get secrets -n openshift-monitoring | grep -E 'observability-alertmanager-accessor|hub-alertmanager-router-ca'
-----

[.console]
[source,bash,subs="attributes+,+macros"]
-----
hub-alertmanager-router-ca-{hub-cluster-id-nosep}            Opaque                                1      97m
observability-alertmanager-accessor-{hub-cluster-id-nosep}   Opaque                                1      97m
-----

[.console-input]
[source,bash,subs="attributes+,+macros"]
-----
# Repeat for both managed clusters ({abi-cluster-kubeconfig} and {ibi-cluster-kubeconfig})
oc --kubeconfig {abi-cluster-kubeconfig} get secret observability-alertmanager-accessor-{hub-cluster-id-nosep} -n openshift-monitoring -o json | \
  jq 'del(.metadata.resourceVersion,.metadata.uid,.metadata.creationTimestamp,.metadata.ownerReferences) | .metadata.name="observability-alertmanager-accessor"' | \
  oc --kubeconfig {abi-cluster-kubeconfig} apply -f -
oc --kubeconfig {abi-cluster-kubeconfig} get secret hub-alertmanager-router-ca-{hub-cluster-id-nosep} -n openshift-monitoring -o json | \
  jq 'del(.metadata.resourceVersion,.metadata.uid,.metadata.creationTimestamp,.metadata.ownerReferences) | .metadata.name="hub-alertmanager-router-ca"' | \
  oc --kubeconfig {abi-cluster-kubeconfig} apply -f -
-----
...
metadata:
  ...
  annotations:
    ...
    lca.openshift.io/apply-label: v1/configmaps/openshift-monitoring/cluster-monitoring-config,v1/secrets/openshift-monitoring/observability-alertmanager-accessor,v1/secrets/openshift-monitoring/hub-alertmanager-router-ca
spec:
  includedNamespaces:
