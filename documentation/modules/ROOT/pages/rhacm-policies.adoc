...
In this section, we provide an introduction to the concepts behind the https://access.redhat.com/documentation/en-us/red_hat_advanced_cluster_management_for_kubernetes/2.5/html/governance/governance[RHACM "Governance" pillar]. Furthermore, we are going to introduce the concept of a policy, which is key for managing the configuration lifecycle of our fleet of clusters.
...
As we will detail in the following section link:managing-at-scale.html#inform-policies["Managing at Scale"], `PolicyGenTemplates` are by default set to generate Policies in inform mode. 
