...
Because of this decoupling, the SiteConfig Operator allow deploying OpenShift clusters using either the {ai-workflow}[Agent Based Instalation] or {ibi-docs}[Image Based Installation] flows depending on the user's needs. See as well, that the operator is the entry point for any GitOps ZTP deployment.
