...
IMPORTANT: If you check the binding rules, you see that we are targeting clusters labeled with `common: "ocp411"`. This label was set in the `SiteConfig` definition in the link:crafting-deployments-iaac.html#siteconfig[previous section].
...
-----
cat <<EOF > ~/5g-deployment-lab/ztp-repository/site-policies/common-policies/common-411.yaml
---
...
IMPORTANT: If you check the binding rules you can see that we are targeting clusters labeled with `group-du-sno: ""`. This label was set in the `SiteConfig` definition in the link:crafting-deployments-iaac.html#siteconfig[previous section].
...
[source,bash,subs="attributes+,+macros"]
-----
cat <<EOF > ~/5g-deployment-lab/ztp-repository/site-policies/group-policies/group-du-sno.yaml
---
apiVersion: ran.openshift.io/v1
kind: PolicyGenTemplate
metadata:
...
[source,bash,subs="attributes+,+macros"]
-----
cat <<EOF > ~/5g-deployment-lab/ztp-repository/site-policies/zone-policies/zone-europe.yaml
---
apiVersion: ran.openshift.io/v1
kind: PolicyGenTemplate
metadata:
...
[source,bash,subs="attributes+,+macros"]
-----
cat <<EOF > ~/5g-deployment-lab/ztp-repository/site-policies/site-policies/site-sno2.yaml
---
apiVersion: ran.openshift.io/v1
kind: PolicyGenTemplate
metadata:
...
We need to create the required kustomization as we did for SiteConfigs. In this case, policies also require a namespace where they will be created. Therefore, we will create the required namespace and the kustomization.
