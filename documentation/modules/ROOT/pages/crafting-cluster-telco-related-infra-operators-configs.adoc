...
In the previous section, we created the infrastructure as code (IaC) for our `SNO2` cluster using the `ClusterInstance` object. In this section, we are going to create the configuration as code (CaC) for our clusters by using multiple `PolicyGenTemplate` objects.
