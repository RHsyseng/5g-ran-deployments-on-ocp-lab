...
As was detailed in the link:ztp-workflow.html#abi-deployment[Agent Based Installation] section, this installation method combines the ease of use of the Assisted Installation service with the ability to run offline and also optionally generate or accept link:ztp-intro.html[Zero Touch Provisioning] (ZTP) custom resources. In this part of the lab we are going to:

* Create the prerequisites for the SNO cluster installation.
* Create the definition of the SNO cluster as a `ClusterInstance` CR.
* Include any extra configuration required for our telco 5G RAN specific use case.
* Commit all those manifests into the `ztp-repository` Git repo where the link:ztp-intro.html[OpenShift GitOps] operator will automatically apply those changes.

[#baremetal-node-details]
== Bare Metal Node Details

The details for our baremetal node that we want to provision as SNO ABI are the ones below:
...
At this point we can commit the changes to the repo. We will push the changes together with cluster and telco related infrastructure operator configurations in the next section.
...
WARNING: It may take a while for the installation to start, wait at least 10 minutes if you see `The cluster is not ready for installation` message or the `SNO ABI` cluster shows up as `draft`.
...
+
5. In order to check the deployment status of the `sno-abi` cluster, click on `sno-abi` and you will get presented this screen:
+
...
+
IMPORTANT: `SNO ABI` cluster installation takes around 50 minutes to complete.
+
...
1. Check the `AgentClusterInstall` for SNO ABI cluster:
+
[.console-input]
[source,bash,subs="attributes+,+macros"]
-----
...
While the `SNO ABI` cluster installs, let's move to the next section where we will learn how to deploy a SNO OpenShift cluster using an link:ztp-workflow.html#ibi-deployment[image-based install flow]. However, first things first, we have to create a seed image from an already deployed link:lab-environment-introduction.html#openshift-seed-sno[Single Node OpenShift seed cluster].
