...
IMPORTANT: ZTP can operate in both connected and disconnected environments. In the connected case clusters have direct access to the Internet. The disconnected case, which uses a private registry accessible within the customer's network, is most common in the telecom environment and will be used in this lab.
