...
LCA operator cannot manage image-based upgrades at scale, in order to run IBU at scale, we leverage another operator, TALM. We will talk about `ImageBasedGroupUpgrade` API and how TALM orchestrates such upgrades in the TALM section of the lab.