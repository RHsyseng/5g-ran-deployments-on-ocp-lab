...
LCA operator cannot manage image-based upgrades at scale, in order to run IBU at scale, we leverage another operator, TALM. We will talk about `ImageBasedGroupUpgrade` API and how TALM orchestrates such upgrades in the TALM section of the lab. In any case, the recommended upgrade method for Telco 5G RAN SNOs is image-based upgrade flow, concretely leveraging the Image Based Group Upgrade (IBGU) CR provided by TALM.