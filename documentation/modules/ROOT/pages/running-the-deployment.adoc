...
podman run --log-driver=none --rm --tls-verify=false {ztp-sitegenerate-disconnected-image} extract /home/ztp --tar | tar x -C ~/5g-deployment-lab/ztp-pipeline/
-----
...
[source,bash,subs="attributes+,+macros"]
-----
oc login -u admin -p <admin_password> https://api.hub.5g-deployment.lab:6443 --insecure-skip-tls-verify=true
-----
+
