...
podman run --log-driver=none --rm --tls-verify=false infra.5g-deployment.lab:8443/openshift4/ztp-site-generate-rhel8:v4.12 extract /home/ztp --tar | tar x -C ~/5g-deployment-lab/ztp-pipeline/
-----
...
[source,bash,subs="attributes+,+macros"]
-----
oc --kubeconfig ~/5g-deployment-lab/hub-kubeconfig login -u admin -p admin https://api.hub.5g-deployment.lab:6443 --insecure-skip-tls-verify=true
-----
+
...
-----
+
.. Allow `ClusterImageSet` resource to the list of allowed resources
+
[.console-input]
[source,bash,subs="attributes+,+macros"]
-----
