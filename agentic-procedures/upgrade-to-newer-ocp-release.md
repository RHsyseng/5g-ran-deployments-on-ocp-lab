...
- You know the version to be used when generating the seed image (otherwise ask the user)
- User has provided path to the hub cluster kubeconfig with cluster-admin privs
...
>
> | `_attributes.adoc` attribute | IBU variable |
> |---|---|
> | `sno-cluster-version2` | version to be used when creating the seed image |

##### Example building IBU seed image for 4.21

1. Update the seed cluster to the `sno-cluster-version2`.

    ~~~sh
    # Get SNO seed kubeconfig
    oc --kubeconfig </path/to/user/provided/hub/kubeconfig> -n sno-seed extract secret/sno-seed-admin-kubeconfig --to=- > /tmp/sno-seed-kubeconfig
    # Upgrade sno-seed cluster using sno-cluster-version2 (4.21.10 in this example)
    oc --kubeconfig /tmp/sno-seed-kubeconfig adm upgrade --to-image="infra.5g-deployment.lab:8443/openshift/release-images:4.21.10-x86_64" --force
    # Wait for the sno-seed cluster to be updated
    ~~~

2. Once the cluster is updated, generate the seed image

    ~~~sh
    # Add registry credentials
    cat <<EOF | oc --kubeconfig /tmp/sno-seed-kubeconfig apply -f -
    ---
    apiVersion: v1
    kind: Secret
    metadata:
    ...
    EOF
    # Run seed generation (use sno-cluster-version2 for the seedImage tag)
    cat <<EOF | oc --kubeconfig /tmp/sno-seed-kubeconfig apply -f -
    ---
    ...
    metadata:
      name: seedimage
    spec:
      seedImage: infra.5g-deployment.lab:8443/ibi/lab5gran:v4.21.10
    EOF
    # Wait for the seedimage to be completed (STATE=SeedGenCompleted)
    oc --kubeconfig /tmp/sno-seed-kubeconfig get seedgen
    ~~~

3. Tell the user they can now skopeo copy the image from `infra.5g-deployment.lab:8443/ibi/lab5gran:v4.21.10` to `quay.io/rhsysdeseng/lab5gran:v4.21.10` (skopeo copy docker://<local-image> docker://<quay-image> --all)